...
### 6. Unity 实时推流（Motion Viewer）

脚本：`stream_unity.py`。对应 Unity 场景 **MotionViewerExample** 里 Hierarchy 的 **Online** 物体：
...
| Port            | `8989`      |
| Connect On Load | 勾选          |
| Timeout Seconds | `60`（够用即可）  |


**启动顺序（必须先 Python 后 Unity Play）：**

1. 在仓库根目录启动推流（会监听并等待连接；**默认循环 + 腕/袋对错标注**）：
  ```bash
   # 好序列（标注应为绿色）
   python experiments/week1_pos_cls/stream_unity.py --combo lw_rp --seq-id 102
   # 坏序列（常见红=Pred 错，蓝/青细线=GT）
   python experiments/week1_pos_cls/stream_unity.py --combo lw_rp --seq-id 12
  ```
2. Unity：确认 Hierarchy 里 **Online** 启用、**Offline** 关掉；**Points**（在 Online 下）保持启用。
3. Inspector 核对 Client：`127.0.0.1` / `8989` / Connect On Load。
4. 点上方 **Play**。若人体被裁切，Game 视图右上角把相机切到 **Front Camera / Side Camera**。
5. 终端出现 `Unity connected` 后 **按 Enter** 开始推流。
6. 结束：终端 `Ctrl+C`，再在 Unity 停 Play。

标注（混合）：

- **手部大球** + **口袋粗射线**：绿=Pred 对，红=Pred 错
- **小蓝 / 青**：仅 Pred 错时标 GT（手=球，口袋=细射线）

常用参数：`--port 8989`、`--fps 30`、`--start/--end`、`--once`、`--auto-start`、`--no-markers`。
