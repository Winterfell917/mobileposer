...
>
> **对外口径（2026-08-16 起）：** \(R_{BS}\) **窗内恒定、序列内可变**（90 帧窗独立采样）。下方第 1–3 步结果表均为该协议、第 1 步重训后重评。  
...
| 朝向 | **未知**：\(R_{MS}=R_{MB}R_{BS}\)，\(a_M\) 不拧；\(R_{BS}\) **窗内恒定、序列内可变** |
...
| 朝向 | **忽略**（不做 mount offset） | **未知** \(R_{BS}\sim\pm 45^\circ\)，窗内恒定 |
...
| **Pred-seq** | 第 1 步 **90 帧段内**多数投票槽位 → Week2 dual \(\hat{R}_{SB}\)（段内平均）→ 校准 → pose |
| **GT-slot** | 真值位置喂 Week2（本协议上界 ≈ Week2 dual Learned） |
| **Oracle** | 完美 \(R_{SB}\) |

评测日期：2026-08-16（窗内恒定 / 分段注入）。W=90；AMASS / IMUPoser 各 12 序列、48 combo-run。  
...
|------|--------:|-----------------:|-----------:|-----------:|---------:|-----------------:|-----------:|-----------:|
| AMASS | 15.38 | **13.89** | 13.80 | 13.12 | 30.78 | **27.58** | 27.36 | 25.78 |
| IMUPoser | 11.33 | **7.21** | 6.95 | 5.46 | 21.92 | **15.97** | 15.27 | 12.20 |

单位：位置 cm / 角度 °。SIP / mesh 同序：AMASS None 34.5° / 18.8 cm → Pred 30.9° / 17.1 cm；IMUPoser None 23.8° / 13.7 cm → Pred 16.4° / 8.9 cm。

### IMUPoser 分组合位置 cm

| combo | None | Pred-seq | GT-slot | Oracle |
|-------|-----:|---------:|--------:|-------:|
| lw_lp_h | 11.68 | **7.17** | 6.89 | 5.42 |
| lw_rp_h（官方训配） | 10.15 | **6.95** | 6.92 | 5.33 |
| rw_lp_h | 12.46 | **8.02** | 7.44 | 5.55 |
| rw_rp_h | 11.03 | **6.71** | 6.53 | 5.54 |

AMASS 四分位 Pred 挤在 13.8–14.0 cm（None 15.2–15.7）；Pred-seq 与 GT-slot 几乎逐组合重合。
...
|--|-------------------:|--------------:|---------------:|---------------:|
| AMASS pos cm | **13.53** | 13.80 | 13.89 | 13.12 |
| AMASS ang ° | 26.80 | 27.36 | 27.58 | 25.78 |
| IMUPoser pos cm | **6.87** | 6.95 | 7.21 | 5.46 |
| IMUPoser ang ° | 15.02 | 15.27 | 15.97 | 12.20 |
...
1. **未知朝向闭环成立。** None > Pred-seq ≥ GT-slot ≥ Oracle。合成位置 15.4→13.9 cm（接近 Oracle 13.1）；真机 **11.3→7.2 cm**（Oracle 5.5），角度 21.9°→16.0°。
2. **级联几乎不比「位置已知」差。** AMASS Pred 与 GT-slot 差 **0.09 cm**（序列 Joint 0.98）。真机差 **0.27 cm**（序列 Joint 0.83）；官方组合 `lw_rp_h` 上 Pred 6.95 vs GT-slot 6.92（约 0.3 mm）。
3. **真机收益远大于合成。** 与 Week2 相同：acc 未拧，None 在合成上本就不差；真机未校准的 \(R_{MS}\) 会把 \(\pm 45^\circ\) 外参送进姿态网。
4. **四组合都能用。** 官方训配 `lw_rp_h` 不是唯一能跑的；另外三组 Pred 也在 6.7–8.0 cm。解读时仍以 `lw_rp_h` 为主。
5. **瓶颈在姿态网与真机域，不在第 1 步。** 再抠位置分类对姿态只有数毫米量级；真机 GT-slot 已在 6.95 cm，离 Oracle 5.5 cm 只剩外参网域差。相对 Week2 Learned（6.87 cm），未知位置只多约 **0.3 cm**。

### 简要结论
- 位置 Pred → \(R_{SB}\) → \(R_{MB}\) → MobilePoser **可以接上**：真机位置误差从 **11.3 cm 降到 7.2 cm**，接近位置已知的 Week2 Learned（6.9 cm）与 Oracle（5.5 cm）。
- 不必为姿态这一步重训任何网络。未知朝向的主要伤害已经被第 2 步的 \(\hat{R}_{SB}\) 吃掉。
- \(R_{MS}\to\) pose 基线（None）在真机上明显更差，可视化见 `step3_mesh_seq*` / pyrender 视频。

## 已知注意点
- **口径：** 现行是 \(R_{BS}\) 窗内恒定（`offset_per: window`）。旧整段协议数字（AMASS Joint 0.832、真机姿态 11.6→6.6 cm 等）不要写进本周汇报。
