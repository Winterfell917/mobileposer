"""PyTorch Dataset / DataLoader for Week2 R_SB regression."""
...
class RotExtDataset(Dataset):
    def __init__(
        ...
        self.slot = data["slot"].long()
        self.r_sb = data["r_sb"].float()  # [N, 3, 3]
        self.r_sb_6d = data["r_sb_6d"].float()  # [N, 6]
    ...
    def __getitem__(
        ...
        return x, self.slot[idx], self.r_sb[idx], self.r_sb_6d[idx]
...
def make_loader(
    ...
    ds = RotExtDataset(pt_path, norm_stats=stats, normalize=True)
    return DataLoader(
        ds,
